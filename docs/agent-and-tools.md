...
| `search-history` | Semantic search over past chats and memories, on-device | — |
| `run-javascript` | Runs JavaScript in a sandboxed embedded engine | — |
| `create-file` | Saves a text-based (or base64) file the user can open/share | — |
| `generate-pdf` | Lays out a paginated PDF from markdown-lite text | — |
| `render-html` | Displays a self-contained HTML view inline, sandboxed | — |
...
- **`search-history`** is the retrieval layer over everything the app already stores: past
  conversations and memory facts are chunked, embedded on-device, and searched by cosine
  similarity plus a keyword bonus (see [memory.md](memory.md#semantic-recall)). The active
  conversation is excluded — it is already in context. Nothing about the search leaves the
  phone.
- **`run-javascript`** executes in an embedded Rhino interpreter with no Java bridge (every
  class is hidden by a `ClassShutter`, and the scope is initialized without LiveConnect), no
  network, and no filesystem beyond a `saveFile` helper that writes into the app-private
  artifacts directory. An instruction observer enforces a 10-second wall-clock deadline, so
  `while(true)` dies instead of hanging the run. Rhino runs interpreted — Android cannot
  load its generated bytecode, and the observer needs interpreted mode anyway.
- **`create-file`**, **`generate-pdf`**, and **`render-html`** all write into
  `filesDir/artifacts` and return an `artifact://` reference inside the tool result. The
  transcript UI parses those references back out of the stored tool records, so chips and
  inline views re-render after a restart with no schema change. PDFs are laid out with the
  platform `PdfDocument` (headings, bullets, code blocks, page breaks) — no HTML rendering
  involved. HTML views render in a WebView locked down to a drawing surface: JavaScript on,
  network loads blocked, file/content access off, no JS bridges, navigation swallowed;
  the user can expand a view fullscreen or share the underlying file.
